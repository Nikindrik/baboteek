    ...
    def expect(self, token_type: str, value: str | None = None) -> Token:
        ...
        if token is None:
            raise ParseError(  # noqa: TRY003
                f"Unexpected EOF. Expected {token_type} '{value or ''}'",
                None,
            )
        ...
        raise ParseError(f"Expected {expected}, but got {token.type_} '{token.value}'", token)  # noqa: TRY003
...
class ExpressionParser:
    ...
    def parse_factor(self) -> None:
        ...
        if not token:
            raise ParseError("Unexpected EOF in expression", None)  # noqa: TRY003
        ...
        match (token.type_, token.value):
            ...
            case ("ADD_OP", "+" | "-"):
                ...
                self.parse_factor()
            case _:
                raise ParseError(f"Unexpected token in expression: '{token.value}'", token)  # noqa: TRY003
    ...
    def _parse_number(self) -> None:
        ...
        if (
            ...
            raise ParseError(f"Invalid number format: '{token.value}'", token)  # noqa: TRY003
...
class StatementParser:
    ...
    def parse_statements(self) -> None:  # noqa: C901
        while token := self.stream.current():
            match (token.type_, token.value):
                ...
                case ("KEYWORD", "end"):
                    return
                case _:
                    raise ParseError(f"Unexpected statement start: '{token.value}'", token)  # noqa: TRY003
